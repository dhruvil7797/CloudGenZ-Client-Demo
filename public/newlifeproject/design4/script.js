...
dbs.forEach(b=>{b.addEventListener('click',()=>{dbs.forEach(x=>x.classList.remove('active'));b.classList.add('active');a=parseInt(b.textContent.replace('$',''));di.value='';db.textContent=`Give $${a} ♥`;})});
di.addEventListener('input',()=>{const v=parseInt(di.value);if(v>0){dbs.forEach(x=>x.classList.remove('active'));a=v;db.textContent=`Give $${v} ♥`;}});
db.addEventListener('click',()=>alert(`Thank you for your generous gift of $${a}!`));
